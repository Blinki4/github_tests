...
class TestRepositoryUI:
    ...
    def test_redirect_to_new_repo_page_ui(self, create_repository_ui, new_repo_page, delete_repository_req):
    ...
    def test_new_repo_title_displayed_ui(self, create_repository_ui, repo_page, delete_repository_req):
        assert repo_page.repo_title_text == os.getenv('REPO_NAME')

    @pytest.mark.ui
    @allure.title('Создание нового репозитория с существующим именем')
    def test_new_repo_with_existing_name_ui(self, new_repo_page):
    ...
    def test_new_private_repo_ui(self, new_repo_page, repo_page, delete_repository_req):
    ...
    def test_delete_repo_ui(self, create_repository_ui, repo_page):
